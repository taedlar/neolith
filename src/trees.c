/*  $Id: trees.c,v 1.1.1.1 2002/11/23 07:57:08 annihilator Exp $
...
parse_node_t *
make_branched_node (short kind, char type, parse_node_t * l, parse_node_t * r)
{
  parse_node_t *ret;
...
parse_node_t *
binary_int_op (parse_node_t * l, parse_node_t * r, char op, char *name)
{
  parse_node_t *ret;
...
parse_node_t *
make_range_node (int code, parse_node_t * expr, parse_node_t * l,
		 parse_node_t * r)
{
  ...
  if (exact_types)
	...
	default:
	  if (expr->type & TYPE_MOD_ARRAY)
	    ...
	    {
	      type_error ("Bad type of argument used for range: ",
			  expr->type);
...
parse_node_t *
insert_pop_value (parse_node_t * expr)
{
...
parse_node_t *
optimize_loop_test (parse_node_t * pn)
{
  parse_node_t *ret;
  ...
  if (IS_NODE (pn, NODE_BINARY_OP, F_LT) &&
      ...
      if (IS_NODE (pn->r.expr, NODE_OPCODE_1, F_LOCAL))
	{
	  CREATE_OPCODE_2 (ret, F_LOOP_COND_LOCAL, 0,
			   pn->l.expr->l.number, pn->r.expr->l.number);
	}
      else if (pn->r.expr->kind == NODE_NUMBER)
	{
	  CREATE_OPCODE_2 (ret, F_LOOP_COND_NUMBER, 0,
			   pn->l.expr->l.number, pn->r.expr->v.number);
	}
      else
	ret = pn;
    }
