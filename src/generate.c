/*  $Id: generate.c,v 1.1.1.1 2002/11/23 07:57:08 annihilator Exp $
...
static void
lpc_tree (parse_node_t * dest, int num)
{
  parse_node_t *pn;
...
static void
lpc_tree_number (parse_node_t * dest, int num)
{
  CREATE_NUMBER (dest->v.expr, num);
}

static void
lpc_tree_real (parse_node_t * dest, float real)
{
  CREATE_REAL (dest->v.expr, real);
}

static void
lpc_tree_expr (parse_node_t * dest, parse_node_t * expr)
{
...
static void
lpc_tree_string (parse_node_t * dest, char *str)
{
  CREATE_STRING (dest->v.expr, str);
}

static void
lpc_tree_list (parse_node_t * dest, parse_node_t * expr)
{
  parse_node_t *pn;
  int num = 0;
...
void
optimizer_start_function (int n)
{
  if (n)
    {
      last_local_refs =
...
dump_tree (parse_node_t * expr)
{
  ...
  switch (expr->kind)
    ...
    case NODE_BRANCH_LINK:
      ...
      break;
    case NODE_CALL_2:
      printf ("(%s %i %i %i ", instrs[expr->v.number].name,
