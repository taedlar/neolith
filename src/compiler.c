/*  $Id: compiler.c,v 1.1.1.1 2002/11/23 07:57:08 annihilator Exp $
...
int
add_new_function_entry ()
{
  int index =
    mem_block[A_FUNCTION_FLAGS].current_size / sizeof (unsigned short);
 ...
 */
void
copy_function (program_t * prog, int index, program_t * defprog, int defindex,
	       int typemod)
{
  ident_hash_elem_t *ihe;
...
{
  inherit_t *ip;
  int num_inherits = 0, super_length = 0;
 ...
 */
int
define_new_function (char *name, int num_arg, int num_local, int flags,
		     int type)
{
  ...
  if (!funp)
	...
	sizeof (compiler_function_t);
      funp =
	(compiler_function_t *) allocate_in_mem_block (A_COMPILER_FUNCTIONS,
						       sizeof
						       (compiler_function_t));
...
get_type_name (char *where, char *end, int type)
{
  ...
  if (type & TYPE_MOD_CLASS)
    ...
    {
      DEBUG_CHECK (type >=
		   sizeof compiler_type_names / sizeof compiler_type_names[0],
		   "Bad type\n");
...
clean_parser ()
{
   ...
   */
  for (i = 0;
       i < mem_block[A_COMPILER_FUNCTIONS].current_size / sizeof (*funp); i++)
    {
