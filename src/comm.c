/*  $Id: comm.c,v 1.1.1.1 2002/11/23 07:57:08 annihilator Exp $
...
static char telnet_sb_lm_mode[] =
  { IAC, SB, TELOPT_LINEMODE, LM_MODE, MODE_ACK, IAC, SE, 0 };
static char telnet_sb_lm_slc[] = { IAC, SB, TELOPT_LINEMODE, LM_SLC, 0 };
static char telnet_se[] = { IAC, SE, 0 };
			  ...
			  for (nslc = 0, j = 2; j < ip->sb_pos - 3; j += 3)
			      ...
			      else
				{
				  switch (slc[SLC_FLAGS] & SLC_LEVELBITS)
				    ...
				    case SLC_VARIABLE:
				    case SLC_CANTCHANGE:
				      /* 只允許使用 ASCII 0-32 及 127 作為控制字元, 以免中文吃字 */
  ...
  if (i == max_users)
    {
      if (all_users)
	{
	  all_users = RESIZE (all_users, max_users + 50, interactive_t *,
			      TAG_USERS, "new_user_handler");
	}
      else
	{
	  all_users = CALLOCATE (50, interactive_t *,
				 TAG_USERS, "new_user_handler");
	}
