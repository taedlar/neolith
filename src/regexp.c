/*  $Id: regexp.c,v 1.1.1.1 2002/11/23 07:57:08 annihilator Exp $
 ...
 */
regexp *
regcomp (unsigned char *exp, int excompat)	/* \( \) operators like in unix ex */
{
  register regexp *r;
  ...
  exp2 = (short *)
    DXALLOC ((strlen ((char *) exp) +
	      1) * (sizeof (short[8]) / sizeof (char[8])), TAG_TEMPORARY,
	     "regcomp: 1");
	...
	case '\\':
	  switch (c = *scan++)
	    ...
	    case 0:
	      FREE (exp2);
	      FAIL
		("Regular expression cannot end with '\\'.  Use \"\\\\\".\n");
	      break;
	    case '(':
	    case ')':
