/*  $Id: backend.c,v 1.2 2002/11/25 11:11:05 annihilator Exp $
...
/*  backend()
 * 
 *  這個函數是系統的主迴圈，控制整個系統的 I/O polling 和 update。
 */
int eval_cost;
...
backend ()
{
  ...
  while (1)
      ...
      if (slow_shut_down_to_do)
	{
      ...
      if (heart_beat_flag)
	{
	  /* 如果還有 heart_beat 沒有做完，把 timeout 設定成 0，所以
	   * select 不會等待 I/O 的發生而立即返回，以便我們處理尚未
	   * 完成的 heart_beat
	   */
	  timeout.tv_sec = 0;
	  timeout.tv_usec = 0;
