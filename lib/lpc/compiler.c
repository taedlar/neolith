...
char *
get_two_types (char *where, char *end, int type1, int type2)
{
 ...
 */
void
copy_variables (program_t * from, int type)
{
  int i;
 ...
 * This index is also referred as "runtime index" in other parts of the code.
 */
...
int
lookup_class_member (int which, char *name, char *type)
{
  int i;
  class_def_t *cd;
  ...
  cme =
    ((class_member_entry_t *) mem_block[A_CLASS_MEMBER].block) + cd->index;
  for (i = 0; i < cd->size; i++)
    {
...
parse_node_t *
reorder_class_values (int which, parse_node_t * node)
{
  class_def_t *cd;
  parse_node_t **tmp;
  int i;
...
void
copy_structures (program_t * prog)
{
  class_def_t *sd;
  ...
  char *str;
  int sm_off =
    mem_block[A_CLASS_MEMBER].current_size / sizeof (class_member_entry_t);
  int sd_off = mem_block[A_CLASS_DEF].current_size / sizeof (class_def_t);
  int i, num = 0;

  sd =
    (class_def_t *) allocate_in_mem_block (A_CLASS_DEF,
                                           prog->num_classes *
                                           sizeof (class_def_t));
  ...
  sme =
    (class_member_entry_t *) allocate_in_mem_block (A_CLASS_MEMBER,
                                                    sizeof
  ...
  while (num--)
    {
      ...
      sme[num].name =
        store_prog_string (prog->strings[prog->class_members[num].name]);
    }
}
...
static void
remove_overload_warnings (char *func)
{
  ovlwarn_t **p;
  ovlwarn_t *tmp;
...
void
overload_function (program_t * prog, int index, program_t * defprog,
                   int defindex, int oldindex, int typemod)
{
  int alias;
 ...
 */
int
copy_functions (program_t * from, int typemod)
{
  ...
  if (from->num_functions_defined &&
      (from->function_table[from->num_functions_defined - 1].name[0] ==
...
int
compatible_types2 (int t1, int t2)
{
#ifdef OLD_TYPE_BEHAVIOR
  /* The old version effectively was almost always was true */
  return 1;
 ...
 */
static int
find_matching_function (program_t * prog, char *name, parse_node_t * node)
{
...
void
arrange_call_inherited (char *name, parse_node_t * node)
{
  inherit_t *ip;
  int num_inherits = 0;
          ...
          if (!(flags & NAME_PROTOTYPE))
            {
              if (fundefp->num_arg != num_arg && !(funflags & NAME_VARARGS))
                yyerror
                  ("Number of arguments disagrees with previous definition.");
...
int
define_variable (char *name, int type, int hide)
{
  variable_t *dummy;
  int n;
  ident_hash_elem_t *ihe;
...
int
define_new_variable (char *name, int type)
{
  int n;
  unsigned short *tp;
  char **np;
...
char *
get_type_name (char *where, char *end, int type)
{
  int pointer = 0;
  ...
  if (type & TYPE_MOD_CLASS)
    {
      ...
      if (current_file)
        where =
          strput (where, end,
                  PROG_STRING (CLASS (type & ~TYPE_MOD_CLASS)->name));
    }
  else
    {
...
int
validate_function_call (int f, parse_node_t * args)
{
      ...
      if (num_var)
        {
          p =
            strput (buff, end,
                    "Illegal to pass a variable number of arguments to non-varargs function ");
          p = strput (p, end, funp->name);
          p = strput (p, end, "\n");
          yyerror (buff);
        }
  ...
  if ((prog = FUNCTION_PROG (f)))
  ...
  else
    {
      ...
      int start =
        *((unsigned short *) mem_block[A_ARGUMENT_INDEX].block + which);
      if (start != INDEX_START_NONE)
        arg_types =
          (unsigned short *) mem_block[A_ARGUMENT_TYPES].block + start;
    }
          ...
          for (argn = 0; argn < num; argn++)
            {
              ...
              if (!enode->v.expr)
                break;
              tmp = enode->v.expr->type;
              for (i = 0; !compatible_types (tmp, argp[i]) && argp[i] != 0;
                   i++)
                ;
  ...
  if (mem_block[A_ARGUMENT_TYPES].current_size)
    {
      ...
      for (i = 0; i < new_num; i++)
        dest[i] =
          *((unsigned short *) mem_block[A_ARGUMENT_INDEX].block + temp[i]);
...
compress_function_tables ()
{
  ...
  if (j + n_def == 0)
    ...
    {
      p =
        (runtime_function_u *) DXALLOC ((j + n_def) *
                                        sizeof (runtime_function_u),
                                        TAG_COMPILER,
...
int
case_compare (parse_node_t ** c1, parse_node_t ** c2)
{
...
int
string_case_compare (parse_node_t ** c1, parse_node_t ** c2)
{
  int i1, i2;
  char *p1, *p2;
...
void
prepare_cases (parse_node_t * pn, int start)
{
