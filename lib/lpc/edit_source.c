/* edit_source.c - code-generation utility for LPC efun tables and predefined macros.
 *
 * Reads two inputs and writes five generated headers to ${CMAKE_BINARY_DIR}:
 *
 *   Input 1 — options.h (generated from options.h.in by CMake configure_file)
 *     Scanned for #define macros; used to populate efuns_option.h with the
 *     predefined macro table that the LPC preprocessor exposes to mud code.
 *
 *   Input 2 — func_spec.i (generated by C preprocessor from func_spec.c.in)
 *     Parsed by make_func.y (Bison); used to generate the efun/opcode tables:
 *       efuns_opcode.h      — F_* opcode and efun number #defines
 *       efuns_prototype.h   — void f_<name>(void) declarations
 *       efuns_vector.h      — efun_table[] function-pointer array
 *       efuns_definition.h  — predefs[] keyword table, efun_arg_types[]
 *
 * Build pipeline (lib/lpc/CMakeLists.txt):
 *   1. configure_file(func_spec.c.in)  →  func_spec.c   (CMake configure step)
 *   2. C preprocessor on func_spec.c   →  func_spec.i   (custom command)
 *   3. edit_source POST_BUILD          →  efuns_*.h     (this program)
 */
