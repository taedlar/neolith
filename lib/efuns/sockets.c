...
f_socket_create (void)
{
  ...
  if (check_valid_socket ("create", (socket_fd_t)-1, current_object, "N/A", -1))
    {
      if (num_arg == 2)
...
f_socket_bind (void)
{
  ...
  fd = (int)(sp - 1)->u.number;
  get_socket_address (fd, addr, &port);

  if (VALID_SOCKET ("bind"))
    {
      i = socket_bind (fd, (int)sp->u.number);
      (--sp)->u.number = i;
    }
  else
    {
...
f_socket_accept (void)
{
  ...
  get_socket_address (fd = (int)(sp - 2)->u.number, addr, &port);
...
f_socket_release (void)
{
  ...
  fd = (int)(sp - 2)->u.number;
  get_socket_address (fd, addr, &port);

  (sp - 2)->u.number = VALID_SOCKET ("release") ?
    socket_release ((int)(sp - 2)->u.number, (sp - 1)->u.ob, sp) : EESECURITY;
...
f_socket_acquire (void)
{
  ...
  (sp - 3)->u.number = VALID_SOCKET ("acquire") ?
    socket_acquire ((int)(sp - 3)->u.number, (sp - 2), (sp - 1), sp) : EESECURITY;
  pop_3_elems ();
}
#endif
...
f_socket_error (void)
{
  put_constant_string (socket_error ((int)sp->u.number));
}
#endif
