  ...
  do
    {
      ...
      new_ln->l_stat = 0;
      strncpy (new_ln->l_buff, str, len);	/* build new line */
      new_ln->l_buff[len] = EOS;
      nxt = getnextptr (P_CURPTR);	/* get next line */
      relink (P_CURPTR, new_ln, new_ln, nxt);	/* add to linked list */
      relink (new_ln, nxt, P_CURPTR, new_ln);
      P_LASTLN++;
      P_CURLN++;
      P_CURPTR = new_ln;
      str = cp + 1;
...
is a regular expression, so things like .*][() have to be preceded by a \\.\n");
    ...
    case '?':
      ED_OUTPUT (ED_DEST, "Command: ?   Usage: ?pattern\n");
      ED_OUTPUT (ED_DEST,
                 "finds the last occurence of 'pattern' before the current line.  Note that pattern\n\
is a regular expression, so things like .*][() have to be preceded by a \\.\n");
      break;
