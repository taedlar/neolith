 ...
 *  @param start The line number to start reading from (1-based).
 *  @param len The number of lines to read. If 0, read the entire file.
 */
      ...
      if (len && !feof (f))
        {
          ...
          fclose (f);
          FREE_MSTR (str);
          opt_trace (TT_EVAL|0, "requested lines exceed __MAX_READ_FILE_SIZE__\n");
          return 0;
        }
      *p2 = '\0';
