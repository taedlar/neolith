 ...
 *
 * This file is used to specify the operators and efuns, ingested by the
 * `edit_source` tool. It is used in the build process to generate the
 * efun_*.h header files that are used by the driver. This can be viewed as
 * the specification of the LPC language's built-in operators and functions.
 *
 * The specification can be customized with the following headers:
 * - <config.h>
 *   Contains configuration options generated by the CMake configuration step.
 *   This is the primary way to customize Neolith's features and efuns.
 * - "options.h"
 *   The legacy configuration header used in MudOS still used (for reference).
 *
 * WARNING: If you change this file, CMake configuration settings or options.h,
 * the driver may build a different set of opcndes and efuns that may NOT be
 * compatible with previously saved binaries. The driver_id should be bumped
 * in such cases to force recompilation of all affected binaries.
 */
#ifdef	HAVE_CONFIG_H
#include <config.h>
#endif	/* HAVE_CONFIG_H */
 ...
 *
 * Each operator declaration here will cause an opcode (F_*) to be defined in
 * generated `efuns_opcode.h`.
 */
 ...
 * An argument can have different types with the syntax 'type1 | type2 | ...'.
 ...
 * ones to the top).
 *
 * The opcprof() efun could help you find out which
