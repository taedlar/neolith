...
format as [time()](time.md)).  The size_of_file element is the same
value that is returned by [file_size()](file_size.md); the size of the file
in bytes, or -2 if it's a directory.

## SEE ALSO
[file_size()](file_size.md), [stat()](stat.md), [time()](time.md)
