...
- **LPC Source Resolution**: Object loading prefers `.lpc` source files and falls back to legacy `.c` files. For example, loading `room/start.c` checks `room/start.lpc` before `room/start.c`.
