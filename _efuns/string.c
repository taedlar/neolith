/*  $Id: string.c,v 1.1.1.1 2002/11/23 07:57:08 annihilator Exp $
...
f_replace_string (void)
{
  ...
  if (rlen <= plen)
    {
      /* replace 比 pattern 短 -> 字串長度不可能伸長，不需重新
       * 配置記憶體 */
  ...
  if (rlen <= plen)
    {
      ...
      if (plen > 1)
	{
	  /* pattern 長度大於 1, 使用 jump table 速度較快 */
	  while (src < flimit)
	    {
      ...
      else
	{
	  /* pattern 長度 == 1, 蠻力法最快 */
	  if (rlen)
	    {			/* 因為 rlen<=plen, 且 plen==1, 所以 rlen==1 */
	      while (*src)
		{
		  if (*src == *pattern)
		    {
